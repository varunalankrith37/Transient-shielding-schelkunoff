...
sigma_pulse = 0.3e-6;       % Standard deviation (0.3 microseconds)

%% Generate Gaussian pulse
v_in = V0 * exp(-((t - t0).^2) / (2 * sigma_pulse^2));
...
thickness = 0.5e-3;           % Shield thickness (1 mm)

%% Create frequency vector
df = 1/t_max;               % Frequency resolution
f = (0:N-1)*df;             % Frequency vector

%% Shield transfer function H(f) - CORRECTED VERSION

% Create frequency vector with negative frequencies properly identified
f_sym = f;
f_sym(f_sym > 1/(2*dt)) = f_sym(f_sym > 1/(2*dt)) - 1/dt;  % Wrap to negative
...
N_fft = length(v_in) + length(h_impulse) - 1;  % Minimum length for linear conv
V_in_freq_padded = fft(v_in, N_fft);           % Zero-pad v_in to N_fft
H_padded = fft(h_impulse, N_fft);              % Zero-pad h_impulse to N_fft
V_out_freq = V_in_freq_padded .* H_padded;
v_out_fft_full = real(ifft(V_out_freq));

% Extract 'same' portion to match conv(...,'same')
...
subplot(3,1,2);
plot(t*1e6, v_out_conv, 'r-', 'LineWidth', 2.5, 'DisplayName', 'conv() method');
hold on;
...
sgtitle('Time-Domain Convolution: Direct vs FFT-Based', 'FontSize', 14, 'FontWeight', 'bold');
...
fprintf('Time-domain convolution methods validated!\n');